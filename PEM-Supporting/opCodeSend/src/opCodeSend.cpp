...
void loop() {
    if (Serial.available()) {
        // If data is available on the serial monitor, transmit it
        String input = Serial.readStringUntil('\n');  // Read until newline character
        rf95.send((uint8_t *)input.c_str(), input.length());
        rf95.waitPacketSent();
        Serial.println("Message sent: " + input);
    } else {
        // Otherwise, receive and process LoRa messages
        uint8_t buf[RH_RF95_MAX_MESSAGE_LEN];
        uint8_t len = sizeof(buf);

        if (rf95.waitAvailableTimeout(3000)) {  // Wait for a message
            if (rf95.recv(buf, &len)) {
                // Convert bytes to String, excluding the null termination
                String message((char *)buf, len);
